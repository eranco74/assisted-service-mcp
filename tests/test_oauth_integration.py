...
def oauth_enabled_app():
    ...
    with patch("assisted_service_mcp.src.oauth.settings.OAUTH_ENABLED", True):
        ...
        async def wrapped_register_handler(request):
            result = await oauth_register_handler(request)
            if isinstance(result, dict):
                return JSONResponse(result)
            return result

        async def wrapped_callback_handler(request):
            result = await oauth_callback_handler(request)
            if isinstance(result, dict):
                return JSONResponse(result)
            return result

        async def wrapped_token_handler(request):
...
class TestOAuthEndpointsIntegration:
    ...
    def test_oauth_token_endpoint_enabled(
        self, oauth_enabled_app
    ):  # pylint: disable=redefined-outer-name
